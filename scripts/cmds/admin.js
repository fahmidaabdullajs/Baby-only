...
module.exports = {
    ...
    onStart: async function ({ message, args, usersData, event, getLang }) {
        ...
        // Restrict "add" and "remove" commands to adminBot users only
        if ((args[0] === "add" || args[0] === "-a" || args[0] === "remove" || args[0] === "-r") 
        ...
        switch (args[0]) {
            ...
            case "list":
            case "-l": {
                // Allow all users to see the list of admins
                const getNames = await Promise.all(config.adminBot.map(uid => usersData.getName(uid).then(name => ({ uid, name }))));
                return message.reply(getLang("listAdmin", getNames.map(({ uid, name }) => `╭‣Name: ${name} 👑\n╰‣Uid: ${uid}\n`).join("\n")));
            }
            default:
