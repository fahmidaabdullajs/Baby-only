...
module.exports = {
  ...
  onStart: async ({ message, args }) => {
    ...
    try {
      const searchResponse = await axios.get(`${global.GoatBot.config.api}api/yts?name=${query}`);
  ...
  onReply: async ({ Reply, message, event }) => {
    ...
    try {
      const downloadResponse = await axios.get(`${global.GoatBot.config.api}api/ytmp3?query=${selectedVideo.id}&format=mp3`);
