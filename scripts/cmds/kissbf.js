module.exports = {
    ...
    onStart: async function ({ api, event, args, Currencies }) { 
        ...
        if (mention.length === 0) {
            ...
            // If only one mention, sender becomes 'two' and the mentioned user is 'one'
            two = senderID;
            one = mention[0];
        } else {
            // If more than one mention, assign the first and second mentions to 'one' and 'two'
            one = mention[1];
            two = mention[0];
        }

        // Handle Currencies if module is available
        ...
        // Create the image and send the response
